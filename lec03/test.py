...
def read_f_bracket(line, index):
    token = {'type': 'LEFT_PAREN'}
    return token, index + 1

def read_l_bracket(line, index):
...
def tokenize(line):
    ...
    while index < len(line):
        if line[index].isdigit():
            ...
            (token, index) = read_f_bracket(line, index)
        elif line[index] == ')':
            (token, index) = read_l_bracket(line, index)
        else:
...
def rpn_algo(tokens):
    ...
    for token in tokens:
        if token['type'] == 'NUMBER': # 数字はそのままoutputに行く
            ...
            while operators and operators[-1]['type'] != 'LEFT_PAREN':
                output.append(operators.pop())
            operators.pop() # LEFT_PARENを消す
